...
% RP: Preco je toto oznacene ako W ked predchadzajuce kapitoly pouzivali e, resp. \bar e?
...
% RP: "mohli by sme získať" znie tak prilis hypoteticky
mohli by sme získať takúto optimalizačnú úlohu
\begin{equation}
	\label{eq:chemostat_opt_general}
	\begin{split}
		\min_{D} &\quad D\left(1-\bar{x}\right), \\
		% RP: Funkcia f nie je dobre zavedena. Potrebujete s na to aby ste ziskali ustaleny stav...ale odkial ta optimalizacia ziska s? Skor by som tu mal 0 = f(x, s, D) a teda je jasne, ze tato rovnica pochadza z dynamickeho modelu (resp. nie je to jasne ale da sa to jasne povedat a porovnat s tym, co bolo povedane v kapitole o vseobecnej optimalizacii)
...
% RP: Tu by malo byt jasne povedane preco sa zaoberate Haldane modelom. Povedali ste, ze zariadenie pracuje podla Monod modelu a teda citatel je tu zmateny, poial mu nepoviete, ze budete predpokladat optimalizacu Haldane modelu a preto ze jeho podmnozinou je Monod model (Ki\to\infty) a preto ze nemate informaciu o  existencii/neexistencii inhibicie.
...
\label{eq:chemostat_opt_w_ss}
	 ...
	 \end{split}
\end{equation}
% RP: Ja by tu este dodal, ze alpha proste predstavuje vahovanie medzi jednotlivymi clenmi ucelovej funkcie.
...
% RP: Ja by som skor povedal, ze budeme uvazovat model v tvare s odchylokymi premennymi.
...
% RP: Nerozmuiem, co chce povedat nasledujuca veta.
...
% RP: Toto je prilis deklaratorne. Tu je skor opacna kauzalita...iteracie robime preto lebo nasa identifikacia nemusi byt vzdy uplne presna. Navyse by sme mohli zohladnit meniace vlastnosti zariadenia.
...
% RP: Preco je toto vhodne zapojenie? Opat prilis deklaratorne.
Vhodnou štruktúrou hybridného modelu na riešenie tejto problematiky je paralelné zapojenie.

% RP: Zislo by sa aj povedat, ze tato situaica je beznejsia v praxi.
...
% RP: Ja by som navrhol sem dat obrazok alebo schemu tohto iteracneho postupu. Opisany je dobre ale pomocou obrazka by ste to uplne podciarkli. Navyse nieco take zrejme budete potrebovat aj v prezentacii.
...
% RP: "dokým" mi nepride velmi po slovensky
% RP: D* v nesimulovanej realite nepoznate, takze taketo ukoncovacie kriterium by nefungovalo. Ja by som povedal, ze moznosti su dve: 1. Budeme proste konvergovat k nejakemu ustalenemu stavu, co bude fungovat za predpokladu, ze hodnota ucelovej funkcie klesa. 2. Budeme zaznamenavat hodnotu ucelovej funkcie a po istom case zistime, ktore D dava najlepsiu hodnotu. Toto D vyhlasime za optimalne a suystem pomocou neho prevadzkujeme. Oba sposoby maju svoje uskalia a najlepsi bude zrejme vhodna kombinacia oboch z nich.
...
% RP: Tu by sa hodil odkaz na zodpovedajucu teoreticku kapitolu.
...
% RP: Mozete spomenut aj nejaku vyhodu, nielen nevyhody. Inak citatel nevidi, preco vlastne takuto metodu spominate.
...
\begin{equation}
	...
	% RP: Nepaci sa mi to oznacenie ()_k. Pride mi, ze to nie je jedznoznacne a zbytocne cloveka matie.
...
\begin{align}
	...
	% RP: Toto som trochu upravil aby sa matematika a text nemiesali ak nevyhnutne nemusia. Da sa s tym este hrat ak sa Vam pozicia "kde" nepaci.
	\text{kde} & \notag\\
...
% RP: idealny = najlepsi, najidealnejsi = najnajlepsi

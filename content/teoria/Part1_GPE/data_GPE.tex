...
% RP: Ja by som toto skor nazval "Graficka ilustracia GOP"
...
% RP: Skor "stanoveny rozsah chyby merania" ako "stanovenú chybu merania"
má stanovenú chybu merania $ e $. Štruktúru modelu procesu, z ktorého sme získali údaje, nepoznáme a preto sa rozhodneme, že
% RP: To je prilis silna skratka, ze ak nepozname model, tak si rovno zvolime linearny. Mohli by sme skor zvolit linearitu ako predpoklad alebo by sme sa mohli pozriet na data usudit pribliznu linearitu z dat (ak mame v datach z nejakeho ustaleneho stavu y=k*u a z dalsieho ustaleneho stavu dostaneme a*y = k*(a*u), tak je proces linearny)
budeme tieto údaje aproximovať lineárnym modelom
\begin{equation}
	\hat{y}(u) = \theta_1 + \theta_2u.
\end{equation}
% RP: Podla akeho vzoru sa sklonouje model? "v duby" alebo "v strojy"? Je to vzor dub alebo v lokale ma vynimku "v modeli"
...
\begin{equation}
	...
	\end{split} 
\end{equation}
% RP: Tu treba jasne uviest Obr. 2.1. a vsetko na nom popisat. Citatel sa len tak sam z obrazka "nenaje". Nie je jasny ani priklad, ktory riesite, takze citatel nevie co by Vam vlastne malo vyjst.
...
\begin{equation}
	\begin{split}
		...
		% RP: Malo by tu byt jasne, ze \hat y je funkciou parametrov. Podobne by to vlastne mohlo byt uz aj v (2.1).
...
% RP: n_y nie je prave intuitivne meno. Preco nie skor n_\theta?
...
% RP: Na rovnice odporucam \eqref, nie \ref
\ref{eq:gpe:general_form}. Preto, skôr ako začneme riešiť úlohu odhadu parametrov, potrebujeme odhadnúť vhodnú štruktúru resp. rád modelu. V tejto časti uvedieme ako riešiť problematiku odhadu minimálneho rádu modelu. Úlohu hľadania adekvátneho maximálneho rádu modelu objasníme v časti \aps{Verifikácia dátových modelov}.

% RP: Z tohto odseku vobec nie je jasne, ako to vlastne funguje. Je to napisane akoby ny bola optimalizacna premenna, ktorej hodnotu nam vrati optimalizacia.
...
% RP: "efektívnejším spôsobom" v porovnani s?
...
% RP: Tusim sa to po slovensky vola "pravdepodobnostna funkcia", nie "funkcia pravdepodobnosti"
...
\end{equation}
% RP: Hodilo by sa sem namiesto x-\mu dat nieco podobne ako 1.14. Moholo by to byt (a v skutocnosti to tak aj predsa je) \prod_i^N \frac{1}{\sigma \sqrt{2\pi}}e^{-\frac{1}{2}\left( \frac{y_i - \hat y_i}{\sigma} \right)^2}
...
\end{equation}
% RP: V predchadzajucej kapitole ste pocet dat oznacili ako N, a tak by bolo celkom slusne to tak zachovat
...
\textbf{BIC}
\newline
% RP: Ja by som to skor prelozil ako "Bayesovske" nie "Bayesove"
...
% RP: "z ... teroii"?
% RP: uff, netusim co je "skreslenie podpornej funkcie". Ak to neviete ani Vy, tak radsej 
...
Pareto front môžeme využiť aj pri verifikácii kvality dátových modelov. Stačí ak zobrazíme niektoré skúmané vlastnosti vybraného súboru modelov, napríklad presnosť a
% RP: Toto nadhodnotenie je tu prilis abstratkne lebo nie je jasne, co presne to znamena. Su dve cesty...bud do detailu vysvetlit overestimation prave tu (toto by som ale nerobil) alebo nadhodnotenie nepomenovavat a skor ho nazvat opisne (nieco ako predikcna kvalita modelu). Mohlo by to byt trenovacia vs. predikcna kvalita modelu.
nadhodnotenie (v zmysle zložitosti modelu, \textit{angl. \aps{overestimation}}), ktoré sa snažíme zlepšiť, optimalizovať. A na základe predchádzajúcej úvahy vyberieme taký model, ktorý bude predstavovať ten najlepší možný kompromis medzi pozorovanou presnosťou modelu a nadhodnotením.

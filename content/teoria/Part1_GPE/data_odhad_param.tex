...
% RP: Je treba definovat vyznam vsetkych pouzitych symbolov pri ich prvom vyskyte.
\begin{itemize}
...
\begin{equation}
	\begin{split}
	% RP: Tu by malo byt skor "x(t_i)" v ucelovke lebo inak nie je jasne co x_i znamena
	% Tato suma \sum_{n=1}^{N} by mala iterator "i", nie "n"
		\hat{\theta} = \arg \min_{\theta} \quad & \sum_{n=1}^{N} \left(y_{i} - x_{i}\right)^2. \\
		% RP: Tu sa vytratilo "(t)" za \dot x 
...
% RP: Takyto necislovany odsek sa da riesit ako \begin{paragraph} alebo ako \begin{subsection*}
\textbf{Rekurzívna metóda najmenších štvorcov}
\newline
% RP: Ma zmysel tu mat tuto podkapitolu? RMNS sa dalej nepouziva.
